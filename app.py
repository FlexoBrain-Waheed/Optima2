...
# ==========================================
    ...
    if lang == "English":
        ...
        <table class="corp-table">
            ...
            <tr><td><strong>Drying Tunnel Capacity</strong></td><td class="highlight-adv">6 Meters (23 Nozzles) w/ iDry</td><td class="highlight-mid">Standard (approx 4-5m)</td><td class="highlight-dis">5.7 Meters (17 Nozzles)</td></tr>
            ...
            <tr><td><strong>Sleeve Ejection System</strong></td><td class="highlight-adv">Fully Automatic Push-off</td><td class="highlight-mid">Semi-Auto/Standard</td><td class="highlight-dis">Basic Ejectors</td></tr>
        </table>
        """, unsafe_allow_html=True)
        ...
        <table class="corp-table">
            ...
            <tr><td><strong>نفق التجفيف (Drying Tunnel)</strong></td><td class="highlight-adv">6 أمتار (23 فوهة) مع تقنية iDry</td><td class="highlight-mid">نفق قياسي (~4-5 متر)</td><td class="highlight-dis">5.7 متر (17 فوهة)</td></tr>
            ...
            <tr><td><strong>نظام إخراج السليفات (Ejectors)</strong></td><td class="highlight-adv">دفع أوتوماتيكي بالكامل</td><td class="highlight-mid">شبه آلي / قياسي</td><td class="highlight-dis">طاردات ميكانيكية أساسية</td></tr>
        </table>
        """, unsafe_allow_html=True)
        ...
        with col2:
            st.markdown('<h4 style="color: #1E3A8A; text-align: center;">👁️ مؤشر بيئة العمل وشفافية المراقبة (%)</h4>', unsafe_allow_html=True)
            df_ergo = pd.DataFrame({
...
# ==========================================
    ...
    if lang == "English":
        ...
        <table class="corp-table">
            ...
            <tr><td><strong>Service Cost Structure</strong></td><td class="highlight-adv">Standardized intervention rates</td><td class="highlight-mid">Premium service tier rates</td><td class="highlight-dis">Premium service tier rates</td></tr>
        ...
        <table class="corp-table">
            ...
            <tr><td><strong>مصادر قطع الغيار</strong></td><td class="highlight-adv">مفتوح المصدر (يُسمح بالشراء المحلي المعتمد)</td><td class="highlight-mid">مقيد (يلزم شراء سليفات الشركة الخاصة)</td><td class="highlight-dis">مقيد (يلزم الشراء من الشركة المصنعة)</td></tr>
            <tr><td><strong>هيكل تكاليف الخدمة</strong></td><td class="highlight-adv">معدلات تدخل وصيانة قياسية</td><td class="highlight-mid">معدلات خدمة ذات تسعير ممتاز (Premium)</td><td class="highlight-dis">معدلات خدمة ذات تسعير ممتاز (Premium)</td></tr>
            <tr><td><strong>ترقية الأصول (Retrofit)</strong></td><td class="highlight-adv">مصممة بقابلية عالية للترقية المستقبلية</td><td class="highlight-mid">مرتبطة بتقنيات الشركة المغلقة</td><td class="highlight-dis">مواصفات صلبة / تكاليف ترقية مرتفعة</td></tr>
        </table>
        """, unsafe_allow_html=True)
...
# ==========================================
    ...
    if lang == "English":
        st.markdown('<div class="executive-title">9. Setup Automation: IRIS vs. smartGPS / Traditional</div>', unsafe_allow_html=True)
        st.markdown("""Both SOMA IRIS and BOBST smartGPS fundamentally rely on embedded RFID chips to transfer offline mounting data to the press. However, the critical differentiation lies in **the measurement methodology at the mounter** and **the OPEX structure of the ecosystem.**""")
        ...
        <table class="corp-table">
            <tr>
                ...
                <th>BOBST smartGPS (Optical)</th>
                <th>W&H (Traditional Optical)</th>
            </tr>
            <tr>
                <td><strong>Data Transfer Tech</strong></td>
                <td class="highlight-adv">Standardized RFID Chip (Open Platform)</td>
                <td class="highlight-mid">Proprietary RFID Chip (Closed Platform)</td>
                <td class="highlight-dis">N/A (Optical mark reading on press)</td>
            </tr>
            <tr>
                <td><strong>Offline Measurement Method</strong></td>
                <td class="highlight-adv">Physical tactile sensors (pedals) measure the actual topography, detecting dirt and damage.</td>
                <td class="highlight-mid">Optical / Laser scanning (cannot physically feel surface anomalies).</td>
                <td class="highlight-dis">Basic camera alignment (No topography data).</td>
            </tr>
            <tr>
                <td><strong>OPEX Impact</strong></td>
                <td class="highlight-adv">Standard sleeves can be used and retrofitted (Low OPEX).</td>
                <td class="highlight-mid">Mandates the continuous purchase of highly expensive proprietary smart sleeves (High OPEX).</td>
                <td class="highlight-dis">Standard sleeves (But high substrate setup waste).</td>
            </tr>
        </table>
        """, unsafe_allow_html=True)
        ...
        df_waste = pd.DataFrame({
            'Setup Method': ['Traditional Optical Setup', 'SOMA IRIS / smartGPS'],
        ...
        st.markdown('<div class="executive-title">9. تكنولوجيا إعداد الطباعة: IRIS مقابل smartGPS والتقليدي</div>', unsafe_allow_html=True)
        st.markdown("""يعتمد كل من نظام SOMA IRIS ونظام BOBST smartGPS أساساً على نفس تقنية نقل البيانات (باستخدام شرائح RFID مدمجة لنقل بيانات المونتاج إلى المطبعة). ومع ذلك، يكمن الفارق الجوهري في **آلية أخذ القياسات الفعلية في الماونتر** و**هيكلية التكاليف التشغيلية (OPEX)** لكل نظام.""")
        
        st.markdown('<h3 style="color: #1E3A8A; margin-top: 20px;">🔍 الآلية التكنولوجية والتشغيلية</h3>', unsafe_allow_html=True)
        st.markdown("""
        <table class="corp-table">
            <tr>
                <th>المعيار</th>
                <th>SOMA IRIS (قراءة طوبوغرافية لمسية)</th>
                <th>BOBST smartGPS (قراءة بصرية)</th>
                <th>W&H (الأنظمة البصرية التقليدية)</th>
            </tr>
            <tr>
                <td><strong>تقنية نقل البيانات</strong></td>
                <td class="highlight-adv">شريحة RFID قياسية (منصة مفتوحة)</td>
                <td class="highlight-mid">شريحة RFID مخصصة (منصة مغلقة/محتكرة)</td>
                <td class="highlight-dis">لا يوجد (قراءة علامات بالمطبعة)</td>
            </tr>
            <tr>
                <td><strong>آلية أخذ القياس (Offline)</strong></td>
                <td class="highlight-adv">مستشعرات لمسية ميكانيكية دقيقة (Pedals) تلامس الكليشيه فعلياً وتكتشف الأوساخ والعيوب.</td>
                <td class="highlight-mid">مسح بصري/ليزر (لا يمكنه التحقق المادي من ملمس أو أوساخ السطح الشفافة).</td>
                <td class="highlight-dis">توجيه بصري أساسي بالكاميرات (بدون بيانات تضاريس).</td>
            </tr>
            <tr>
                <td><strong>الأثر المالي (OPEX)</strong></td>
                <td class="highlight-adv">يمكن استخدام السليفات العادية وتجهيزها بالشرائح بمرونة (تكلفة تشغيل منخفضة).</td>
                <td class="highlight-mid">تُلزم المصنع بشراء سليفات "ذكية" خاصة بأسعار باهظة جداً طوال عمر الماكينة (OPEX عالي).</td>
                <td class="highlight-dis">سليفات عادية (ولكن هدر الخامات أثناء الإعداد مرتفع جداً).</td>
            </tr>
        </table>
        """, unsafe_allow_html=True)
        ...
        df_waste = pd.DataFrame({
            'طريقة الإعداد': ['الضبط التقليدي / البصري', 'نظام SOMA IRIS / smartGPS'],
